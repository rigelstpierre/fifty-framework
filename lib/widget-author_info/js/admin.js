...
_standard_author_info_url = null;
_standard_author_info_src = null;

(function($) {

	// Attach event handlers on initial page load
	$(function() {
		standard_author_info($);
	});
	
	// If the Author Info widget is being activated, we need to setup the event handlers again
	$(document).ajaxSuccess(function(e, xhr, settings) {

		var sWidgetName = 'ffw-standard-author-info';
...
/**
 * Attaches event handlers for Standard's Author Info widget administration
 * panel.
 */
function standard_author_info($) {
	...
	*/
	...
	$('.standard-author-info-wrapper .option .author_info_preview_image_container').unbind('click')
		.click(function(evt) {
		
		evt.preventDefault();
	
		// Identify the input fields for this image and anchor
		_standard_author_info_url = '#' + $(this).siblings('.img_url').attr('id');
		_standard_author_info_src = '#' + $(this).siblings('.img_src').attr('id');
		
		// Show the media uploader
		author_info_show_media_uploader();
		$('#TB_iframeContent').load(function() {
		
			// if the user is uploading a new author info image, we need to poll until we see the form fields
			var author_info_poll = setInterval(function() {
				if($('#TB_iframeContent').contents().find('#media-items').children().length > 0) {
					standard_author_info_hide_unused_fields($, author_info_poll);
				} // end if
			}, 500);
	
			// if they aren't uplaoding, we'll clear the fields on load
			standard_author_info_hide_unused_fields($);	
		});
		
	});
	
	// Delete the adveritsement and clear the input fields when the delete anchor is clicked
	$('.standard-author-info-wrapper .option .img_delete').click(function(evt) {
	
		// Reset the hidden field and hide the preview image
		$(this).siblings('.img_url').val('');
		$(this).siblings('.img_src').val('');
		$(this).siblings('.author_info_preview_image_container').children('img').attr('src', $(this).next().val());
 ...
 */
function standard_author_info_hide_unused_fields($, poller) {
...
} // end standard_author_info_hide_unused_fields
 ...
 */
function author_info_show_media_uploader() {
	...
	window.send_to_editor = function(sHtml) {
		...
		if(jQuery(sHtml).children('img').length === 1) {
			...
			jQuery(_standard_author_info_src).val(sImageUrl);
			jQuery(_standard_author_info_url).val(jQuery(sHtml).attr('href'));
			
		} else {
		
			sImageUrl = jQuery(sHtml).attr('src');		
			jQuery(_standard_author_info_src).val(jQuery(sHtml).attr('src'));		
			
		} // end if/else
		
		// Display the image in the admin area
		jQuery(_standard_author_info_src).siblings('.author_info_preview_image_container').children('img').attr('src', sImageUrl).show();
				
		// Hide the upload anchor, show the delete anchor
		jQuery(_standard_author_info_src).siblings('.img_upload').hide();
		jQuery(_standard_author_info_src).siblings('.img_delete').show();
...
} // end ad_author_info_show_media_uploader